...
// Check if Supabase URL and Key are real and configured
...
// Create public anonymous Supabase client (safe for browser)
...
// ============================================================================
// 15-TABLE SUPABASE SQL SCHEMA & MIGRATION SCRIPT
// ============================================================================
export const SUPABASE_SQL_SCHEMA = `-- ============================================================================
-- EMAUDO SENIOR SECONDARY SCHOOL, EKPOMA, EDO STATE, NIGERIA
-- Complete 15-Table Supabase PostgreSQL Relational Schema & Migration
-- ============================================================================
...
-- 1. PROFILES TABLE (Directly linked to auth.users)
...
CREATE TABLE IF NOT EXISTS public.classes (
  ...
  name TEXT NOT NULL,
  level TEXT NOT NULL,
  arm TEXT NOT NULL DEFAULT 'Gold',
...
-- ============================================================================
-- AUTHENTICATION SYNC TRIGGER (auth.users -> public.profiles)
-- ============================================================================
...
CREATE POLICY "Published Announcements viewable by everyone" ON public.announcements FOR SELECT USING (is_published = true OR public.is_admin());

DROP POLICY IF EXISTS "Events viewable by everyone" ON public.events;
CREATE POLICY "Events viewable by everyone" ON public.events FOR SELECT USING (is_published = true OR public.is_admin());

DROP POLICY IF EXISTS "Gallery viewable by everyone" ON public.gallery;
CREATE POLICY "Gallery viewable by everyone" ON public.gallery FOR SELECT USING (is_published = true OR public.is_admin());
...
-- Student & Parent Data Policies
DROP POLICY IF EXISTS "Students and Parents view student profile" ON public.students;
CREATE POLICY "Students and Parents view student profile" ON public.students FOR SELECT USING (
...
CREATE POLICY "Teachers and Admins view teachers profile" ON public.teachers FOR SELECT USING (
  profile_id = auth.uid() OR public.is_admin() OR true
);

DROP POLICY IF EXISTS "Parents view own parent record" ON public.parents;
CREATE POLICY "Parents view own parent record" ON public.parents FOR SELECT USING (
  profile_id = auth.uid() OR public.is_admin()
);

DROP POLICY IF EXISTS "Authorized users view results" ON public.results;
CREATE POLICY "Authorized users view results" ON public.results FOR SELECT USING (
...
CREATE POLICY "Teachers can record and edit results" ON public.results FOR ALL USING (public.is_teacher() OR public.is_admin());

DROP POLICY IF EXISTS "Authorized users view attendance" ON public.attendance;
CREATE POLICY "Authorized users view attendance" ON public.attendance FOR SELECT USING (
...
CREATE POLICY "Teachers can mark attendance" ON public.attendance FOR ALL USING (public.is_teacher() OR public.is_admin());

DROP POLICY IF EXISTS "Students view assignments for their class" ON public.assignments;
CREATE POLICY "Students view assignments for their class" ON public.assignments FOR SELECT USING (
...
CREATE POLICY "Teachers can create and manage assignments" ON public.assignments FOR ALL USING (public.is_teacher() OR public.is_admin());

DROP POLICY IF EXISTS "Students and Parents view fees" ON public.fees;
CREATE POLICY "Students and Parents view fees" ON public.fees FOR SELECT USING (
  student_id IN (SELECT id FROM public.students WHERE profile_id = auth.uid()) OR
  student_id IN (SELECT s.id FROM public.students s JOIN public.parents p ON s.parent_id = p.id WHERE p.profile_id = auth.uid()) OR
  public.is_admin()
);
...
-- ============================================================================
-- INITIAL SEED DATA FOR EMAUDO SENIOR SECONDARY SCHOOL
-- ============================================================================
...
// ============================================================================
// TYPED SUPABASE API HELPER SERVICES
// ============================================================================
...
export const supabaseService = {
  ...
  auth: {
    async signIn(email: string, password: string) {
      if (!supabase) throw new Error('Supabase is not configured yet. Set VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY in settings.');
      return await supabase.auth.signInWithPassword({ email, password });
    },
    async signUp(email: string, password: string, metadata: { full_name: string; role: string; phone?: string }) {
  ...
  admissions: {
    async submit(payload: {
      ...
      if (!supabase) return null;
      return await supabase.from('admissions').insert([payload]);
    },
    async getAll() {
      if (!supabase) return [];
    ...
    async updateStatus(id: string, status: string) {
      if (!supabase) return null;
      return await supabase.from('admissions').update({ status }).eq('id', id);
    },
  },
  ...
  contact: {
    async submit(payload: {
      ...
      if (!supabase) return null;
      return await supabase.from('contact_messages').insert([payload]);
    },
    async getAll() {
      if (!supabase) return [];
    ...
    async updateStatus(id: string, status: string) {
      if (!supabase) return null;
      return await supabase.from('contact_messages').update({ status }).eq('id', id);
    },
  },
  ...
  announcements: {
    ...
    async create(payload: {
      ...
      if (!supabase) return null;
      return await supabase.from('announcements').insert([payload]);
    },
  ...
  events: {
    async getAll() {
      if (!supabase) return [];
      const { data } = await supabase.from('events').select('*').eq('is_published', true).order('event_date', { ascending: true });
      return data || [];
    },
    async create(payload: {
      title: string;
  ...
  gallery: {
    async getAll() {
      if (!supabase) return [];
      const { data } = await supabase.from('gallery').select('*').eq('is_published', true).order('created_at', { ascending: false });
      return data || [];
    },
    async create(payload: {
      title: string;
      ...
      if (!supabase) return null;
      return await supabase.from('gallery').insert([payload]);
    },
  },
  ...
  students: {
    ...
    async getResults(studentDbId: string, term = 'First Term', session = '2024/2025') {
      if (!supabase) return [];
      const { data } = await supabase
        .from('results')
        .select(`
    ...
    async getAttendance(studentDbId: string) {
      if (!supabase) return [];
      const { data } = await supabase
        .from('attendance')
        .select('*')
    ...
    async getFees(studentDbId: string) {
      if (!supabase) return [];
      const { data } = await supabase
        .from('fees')
        .select('*')
  ...
  classes: {
    async getAll() {
      ...
      const { data } = await supabase
        .from('classes')
        .select('*, teacher:profiles!classes_class_teacher_id_fkey(*)')
        .order('level', { ascending: true });
      return data || [];
    },
    async create(payload: { name: string; level: string; arm: string; category?: string; class_teacher_id?: string; academic_year?: string }) {
      if (!supabase) return null;
      return await supabase.from('classes').insert([payload]);
    },
  },
  ...
  subjects: {
    ...
    async create(payload: { code: string; name: string; level?: string; department?: string; is_core?: boolean }) {
      if (!supabase) return null;
      return await supabase.from('subjects').insert([payload]);
    },
  },
  ...
  teachers: {
    async getAll() {
      if (!supabase) return [];
      const { data } = await supabase.from('teachers').select('*, profile:profiles(*)').order('created_at', { ascending: false });
      return data || [];
    },
    async create(payload: { profile_id: string; employee_id: string; specialization: string; department: string; qualifications?: string }) {
      if (!supabase) return null;
      return await supabase.from('teachers').insert([payload]);
    },
  },
  ...
  parents: {
    async getAll() {
      if (!supabase) return [];
      const { data } = await supabase.from('parents').select('*, profile:profiles(*)').order('created_at', { ascending: false });
      return data || [];
    },
    async create(payload: { profile_id: string; occupation?: string; residential_address?: string; emergency_contact?: string; relationship_type?: string }) {
      if (!supabase) return null;
      return await supabase.from('parents').insert([payload]);
    },
  },
  ...
  results: {
    async saveResult(payload: {
      ...
      if (!supabase) return null;
      return await supabase.from('results').upsert([payload], { onConflict: 'student_id,subject_id,term,session' });
    },
  },
  ...
  attendance: {
    async mark(payload: {
      ...
      if (!supabase) return null;
      return await supabase.from('attendance').upsert([payload], { onConflict: 'student_id,date' });
    },
    async saveBulk(records: Array<{
      student_id: string;
      class_id: string;
      date: string;
      status: 'Present' | 'Absent' | 'Late' | 'Excused';
      ...
      if (!supabase || records.length === 0) return { data: null, error: null };
      return await supabase.from('attendance').upsert(records, { onConflict: 'student_id,date' });
    },
    ...
    async getByDate(date: string) {
      ...
      const { data } = await supabase
        .from('attendance')
        .select('*, student:students(*, profile:profiles(*)), class:classes(*)')
        .eq('date', date);
      return data || [];
    },
  ...
  // Fees Management
  fees: {
    async recordPayment(id: string, amountPaid: number, receiptNumber?: string) {
      if (!supabase) return null;
      return await supabase.from('fees').update({
        amount_paid: amountPaid,
        status: 'Paid',
        receipt_number: receiptNumber || `ESS-REC-${Date.now().toString().slice(-6)}`,
      }).eq('id', id);
    },
  },
  ...
  departments: {
    ...
    async create(payload: { department_name: string; description?: string; head_of_department?: string; tagline?: string }) {
      if (!supabase) return null;
      return await supabase.from('departments').insert([payload]);
    },
  },
  ...
  clubs: {
    async getAll() {
      if (!supabase) return [];
      const { data } = await supabase.from('clubs').select('*').order('club_name', { ascending: true });
      return data || [];
    },
    async create(payload: { club_name: string; description: string; coordinator?: string; category?: string; meeting_day?: string; meeting_time?: string; venue?: string }) {
      if (!supabase) return null;
      return await supabase.from('clubs').insert([payload]);
    },
  },
  ...
  activities: {
    ...
    async create(payload: { activity_name: string; description: string; date?: string; category?: string; target_audience?: string }) {
      if (!supabase) return null;
      return await supabase.from('activities').insert([payload]);
    },
  },
  ...
  timetable: {
    ...
    async getAll() {
      if (!supabase) return [];
      const { data } = await supabase.from('timetable').select('*').order('day', { ascending: true });
      return data || [];
    },
    async create(payload: {
      class_id?: string;
      ...
      if (!supabase) return null;
      return await supabase.from('timetable').insert([payload]);
    },
  },

  // Learning Resources (Video Library & Digital Classroom)
  learningResources: {
    async getAll() {
      if (!supabase) return [];
      const { data } = await supabase
    ...
    async getByClass(classLevel: string) {
      if (!supabase) return [];
      const { data } = await supabase
        .from('learning_resources')
        .select('*')
    ...
    async create(payload: Partial<LearningResource> & {
      title: string;
      subject: string;
      class_level: string;
      topic: string;
      ...
      if (!supabase) return null;
      return await supabase.from('learning_resources').insert([{
        ...payload,
        approved: payload.approved ?? true,
        is_available: payload.is_available ?? true,
      }]).select().single();
    },
    async update(id: string, payload: Partial<LearningResource>) {
      if (!supabase) return null;
    ...
    async delete(id: string) {
      if (!supabase) return null;
      return await supabase.from('learning_resources').delete().eq('id', id);
    },
  },
  ...
  // 2. CBT Exams & Questions & Submissions
  exams: {
    async getAll() {
      if (!supabase) return [];
      const { data } = await supabase
    ...
    async create(payload: any) {
      if (!supabase) return null;
      return await supabase.from('exams').insert([payload]).select().single();
    },
    async update(id: string, payload: any) {
      if (!supabase) return null;
      return await supabase.from('exams').update(payload).eq('id', id).select().single();
    },
    async delete(id: string) {
      if (!supabase) return null;
    ...
    async createQuestion(payload: any) {
      if (!supabase) return null;
      return await supabase.from('questions').insert([payload]).select().single();
    },
    async getSubmissions(examId?: string) {
      if (!supabase) return [];
      let query = supabase.from('student_answers').select('*').order('submitted_at', { ascending: false });
    ...
    async submitExam(payload: any) {
      if (!supabase) return null;
      return await supabase.from('student_answers').insert([payload]).select().single();
    }
  },
  ...
  library: {
    ...
    async create(payload: any) {
      if (!supabase) return null;
      return await supabase.from('library_resources').insert([payload]).select().single();
    },
    async delete(id: string) {
      if (!supabase) return null;
      return await supabase.from('library_resources').delete().eq('id', id);
    }
  },

  // 4. Timetable & Exam Schedule
  timetableManagement: {
    async getWeekly() {
      if (!supabase) return [];
      const { data } = await supabase.from('timetable').select('*').order('period_number', { ascending: true });
      return data || [];
    },
    async getExamSchedule() {
      if (!supabase) return [];
      const { data } = await supabase.from('exam_timetables').select('*').order('date', { ascending: true });
      return data || [];
    },
    async createExamSlot(payload: any) {
      if (!supabase) return null;
      return await supabase.from('exam_timetables').insert([payload]).select().single();
    }
  },
  ...
  achievements: {
    ...
    async create(payload: any) {
      if (!supabase) return null;
      return await supabase.from('achievements').insert([payload]).select().single();
    },
    async delete(id: string) {
      if (!supabase) return null;
  ...
  // 6. Messages & School Communication
  messages: {
    ...
    async markAsRead(id: string) {
      if (!supabase) return null;
      return await supabase.from('messages').update({ is_read: true }).eq('id', id);
    }
  },
  ...
  alumni: {
    ...
    async register(payload: any) {
      if (!supabase) return null;
      return await supabase.from('alumni').insert([payload]).select().single();
    },
    async approve(id: string) {
      if (!supabase) return null;
      return await supabase.from('alumni').update({ approved: true }).eq('id', id);
    }
  }
};

// Aliased for convenience across modules
